pro aper,image,xc,yc,mags,errap,sky,skyerr,phpadu,apr,skyrad,badpix, $
       ...
       CLIPSIG=clipsig, MAXITER=maxiter,CONVERGE_NUM=converge_num,ZMAG=zmag, $
       DZMAG = dzmag
...
;Get radii of sky annulii
 ...
 fmt3 = '(I6,5F15.2,6A,2(/,44x,9A,:))'            ;Print format
...
;
 ...
 if ( Ngood GT 0 ) then begin               ;If YES then compute errors
   ...
   apmag[good] =  zmag-2.5*alog10(apmag[good])  
   endif
 endif  
...
;Print out magnitudes for this star
 ...
 for ii = 0,Naper-1 do $              ;Concatenate mags into a string
    ...
    ms[ii] = string( apmag[ii],'  ',sqrt(magerr[ii]^2+dzmag^2), FORM = fmt)
   ...
   mags[0,i] = apmag  &  errap[0,i]= sqrt(magerr^2+dzmag^2)
 endfor                                              ;i
